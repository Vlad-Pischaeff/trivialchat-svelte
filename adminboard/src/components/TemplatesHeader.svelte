<script>
  import { tooltip } from '../js/__Tooltip';
  const tabs = [
                  { 'tip': "Quick short answers...", 'icon': 'data:image/png;base64,' },
                  { 'tip': "Your notes...", 'icon': 'data:image/png;base64,' },
                ]
  export let idx = 0;

  // $: console.log('tabs...', idx);
</script>

<section class="templates_header">
  {#each tabs as tab, i}
    <div class={idx === i ? "tab tab-selected" : "tab"} >
      <img class={idx === i ? "b-icon" : "w-icon"} src={tab.icon} title={tab.tip} 
            use:tooltip alt='answers' on:click={() => idx = i}/>
    </div>
  {/each}
</section>
